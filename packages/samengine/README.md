...
## Svelte Adapter: matching the generated HTML UI

`samengine/svelte/ui` ships drop-in Svelte components that reproduce the
start screen, settings menu, and fullscreen button `samengine-build`
generates - same layout, same colors, same behavior, driven by the exact
same `buildconfig` you already pass to the CLI/HTML build. They read `config`
directly (`config.htmlMenu.style`, `config.htmlMenu.settings`,
`config.title`, ...) via the shared theme functions in `samengine/config`, so
there is a single source of truth for the look of both hosts - the HTML
build and these Svelte components literally call the same CSS-generating
functions, they never drift apart on their own.

Easiest path - one component that wires up all three plus the audio-unlock
flow, matching the generated page exactly:

```svelte
<script lang="ts">
    import { onMount } from "svelte";
    import { SamEngine } from "samengine/svelte";
    import { SamEngineGameUI } from "samengine/svelte/ui";
    import { config } from "./samengine.config.js"; // same config as samengine-build
...
{#if canvas}
    <SamEngineGameUI {config} {canvas} onstart={() => engine.start()} />
{/if}
```

Or use the pieces individually if you want more control over layout:

```svelte
<script lang="ts">
    import { SamEngineStartScreen, SamEngineSettingsMenu, SamEngineFullscreenButton } from "samengine/svelte/ui";
</script>

<SamEngineStartScreen {config} onstart={() => engine.start()} />
<SamEngineSettingsMenu {config} />
<SamEngineFullscreenButton {config} {canvas} />
```

Notes:

- `SamEngineSettingsMenu` writes through the Settings Runtime
  (`setGameSetting`) instead of `window.__GAMESETTINGS__`, and seeds each
  setting's `default_value` on mount unless you already set one yourself
  (e.g. via `new SamEngine({ settings })`).
- The start screen renders as a `position: fixed` overlay covering the
  viewport (a `.samengine-stage` wrapper) instead of styling `<body>`
  directly the way the generated HTML page does - a component dropped into
  an existing SvelteKit app shouldn't reach out and restyle the whole app's
  `<body>`. Everything else (colors, spacing, the settings popup, the
  fullscreen button) is generated by the identical functions the HTML build
  uses, unchanged.
- These components are entirely optional. If you'd rather build your own UI
  in Svelte from scratch, see the section above - `SamEngine` alone doesn't
  require any of this.
...
### Svelte UI Components (`samengine/svelte/ui`)
- `SamEngineGameUI` - Start screen + settings menu + fullscreen button + audio unlock, combined
- `SamEngineStartScreen` - Title/version/author/description/start button/Markdown notes
- `SamEngineSettingsMenu` - Settings popup, wired to the Settings Runtime
- `SamEngineFullscreenButton` - Fullscreen toggle button
- All read the same `buildconfig` as `samengine-build`, styled via the shared
  functions in `samengine/config` (`getStageCSS`, `getSettingsButtonCSS`, `getFullscreenButtonCSS`, `getMDNotesCSS`)
